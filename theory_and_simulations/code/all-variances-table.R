...
WV <- data.table(population_wavVar = sapply(d[-1], function(x)sum(x^2)/1024), level = 1:10)
...
WV[, meanInd_wavVar  := rowMeans(WV[, !c("level", "population_wavVar")])]

output <- WV[, c("level", "population_wavVar", "meanInd_wavVar")]
