...
rmap <- fread(args[3])
IDS1 <- fread(args[4], header=F, col.names='ID')
IDS2 <- fread(args[5], header=F, col.names='ID')
...
#IDS1 <- fread('hilo/allopatric_mexicana_ids.list', header=F, col.names = 'ID')
#IDS2 <- fread('hilo/allopatric_maize_ids.list', header=F, col.names = 'ID')

pairs <- data.table(expand.grid(IDS1[, ID], IDS2[, ID])) #;pairs <- pairs[1:2]
