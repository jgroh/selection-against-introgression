...
# density are themselves correlated, calculate partial correlations
...
cortbl1P <- gnomP[ID==ID[1], gnom_cor_decomp(data = .SD, chromosome = "chr", signals = c("frq_cds_res", "r_cds_res"))]
setnames(cortbl1P, old = oldnames, new = paste0(oldnames, ".meanFreq_r"))

cortbl1.1P <- gnomP[ID==ID[1], gnom_cor_decomp(data = .SD, chromosome = "chr", signals = c("frq_cds_res", "log10r_cds_res"))]
setnames(cortbl1.1P, old = oldnames, new = paste0(oldnames, ".meanFreq_log10r"))
...
cortbl1G <- gnomG[ID==ID[1], gnom_cor_decomp(data = .SD, chromosome = "chr", signals = c("frq_cds_res", "r_cds_res"))]
setnames(cortbl1G, old = oldnames, new = paste0(oldnames, ".meanFreq_r"))

cortbl1.1G <- gnomG[ID==ID[1], gnom_cor_decomp(data = .SD, chromosome = "chr", signals = c("frq_cds_res", "log10r_cds_res"))]
setnames(cortbl1.1G, old = oldnames, new = paste0(oldnames, ".meanFreq_log10r"))
...
old <- c("rsqrd", "ci95_lower", "ci95_upper")
